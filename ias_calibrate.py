...
def prepare_inputs_for_clip(loaded, clip_id: str, device: str):
    """Build model inputs for an arbitrary clip_id (bypasses R1Adapter's hardcoded CLIP_ID)."""
...
def main():
    ...
    p.add_argument("--config", type=Path, default=Path("/root/oom-free-alpamayo/r1_config.json"))
    ...
    p.add_argument("--output", type=Path, default=Path("/root/calibration.json"))
