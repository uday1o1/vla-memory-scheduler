...
Is this genuinely novel? Yes, narrowly. The mechanism, CPU-GPU layer swapping to fit an oversized model, is not novel: a peer-reviewed system, `oom-free-alpamayo` (IEEE RTCSA 2026), already does it well, and a related system, Nova, applies the same core technique to desktop model serving. What is not found anywhere, after reading five related systems in full text, is making the layer-residency decision itself a live function of a deadline budget and sensed GPU contention, for a VLA model specifically. The closest competitor, a compute-placement approach published in June 2026, is itself genuinely runtime-adaptive, reacting to raw GPU and CPU utilization and queue backlog to migrate a small set of boundary layers. The real differentiation is the problem being solved, load-balancing a model that already fits versus fitting a model that does not fit at all, and the signal driving adaptation, raw utilization versus an explicit deadline budget and sensed contention, not adaptive versus static. This project should not claim it achieves true real-time inference; even the best-resourced 2026 competitor does not reach that bound. Success is framed as relative improvement over the existing static baseline. The codebase being extended is small and peer-reviewed; its documented extension point is verified early in implementation to confirm it is as cleanly isolated in practice as its documentation states.
...
**Named limitation 1.** A close competitor published in June 2026 (literature-survey.md, source 5) addresses the same underlying problem, large driving VLA models against real hardware limits, through a compute-placement split that is itself runtime-adaptive: it defines a state vector of GPU utilization, CPU utilization, and queue backlog, and migrates a bounded set of boundary layers online, validated in real-vehicle experiments running concurrently with Autoware.Universe. This project's contribution is stated precisely relative to it: a memory-residency policy for a model that does not fit in GPU memory at all, reacting to an explicit deadline budget and sensed GPU contention across the full residency decision, not a load-balancing policy for a model that already fits, reacting to raw utilization signals within a bounded migration zone.

**Named limitation 2.** CPU-GPU layer swapping is not itself novel. The claimed contribution is the policy governing residency decisions under deadline and contention pressure.

**Named limitation 3.** The 100ms real-time figure is sourced from Alpamayo's own paper, measured on a full-VRAM GPU, not the swapped system and not a realistic rented GPU. Success criteria are defined relative to the static baseline's own achievable range rather than to this figure directly.
