...
Read at abstract level. The newest and closest competing work; it must be cited and differentiated explicitly. Partitions a driving VLA model's backbone so the language model suffix runs on CPU through an asynchronous cross-frame pipeline, tested on two real driving VLA models under coexistence with the rest of an onboard compute stack. Reports 21.7 to 30.9 percent latency reduction and comparable peak memory reduction. This is a static CPU/GPU compute placement decision, validated once, with no indication that the partition point adapts to a sensed deadline or to a second competing workload at runtime. It provides strong evidence that this problem area is active in 2026, while leaving the present project's specific contribution, a live-adaptive residency policy, open. A full text read is planned before implementation begins, to confirm this distinction holds.
