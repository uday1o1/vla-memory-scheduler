...
Read at abstract level. This paper's contention-generation approach, profiling the system under a dummy GPU workload to produce a controllable, steady contention level, is the technique this project's synthetic contention generator is modeled on.
...
**Summary of the gap.** CPU-GPU layer swapping for oversized models is established (sources 0 and 1). Deadline-aware scheduling exists at the inter-task and inter-robot level (sources 2 and 4), not at the level of memory-residency decisions inside a single model. The most recent related work (source 5) is itself runtime-adaptive, but for a compute-placement problem on a model that already fits, reacting to raw utilization and queue signals to migrate a bounded set of boundary layers. No surveyed source combines deadline awareness, sensed GPU contention, and full intra-model layer-residency decisions for a VLA model that does not fit in memory at all.
