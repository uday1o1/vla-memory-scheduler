...
Contention scenarios: a steady compute-bound load using Artificial Contention Units, and a bursty load injected by a second process at randomized intervals. Both run as an independent process sharing the GPU through CUDA MPS.
...
**Foundation.** The project forks `aveeslab/oom-free-alpamayo`, whose codebase is small and cleanly modular, with a documented adapter interface and a single, already-isolated function implementing the GPU-Resident Layer Decision Policy. The swapping mechanism itself, Sequential and Pipelined Demand Layering, is correct existing infrastructure and is not modified.
...
3. Contention injection: Artificial Contention Units and a bursty-arrival generator, run as an independent process sharing the GPU through CUDA MPS.
...
**Data sources.** The model, `nvidia/Alpamayo-R1-10B`, is open, ungated, and permissively licensed. The codebase is MIT licensed. Neither requires special access. Representative driving-scene input frames come from either NVIDIA's `PhysicalAI-Autonomous-Vehicles` dataset, which is gated by an automatic license agreement rather than manual review, or a community integration connecting Alpamayo to the open CARLA simulator, which avoids the licensed dataset entirely and additionally provides a closed-loop test environment if time permits. Only a small sample of driving frames is needed; the full dataset is not required.
...
**Device available.** A rented A100-class GPU, at a market rate of approximately $1.00 to $1.50 per hour, with root or full-OS access needed for reliable CUDA MPS control. The evaluation is a bounded sweep of baseline against adaptive policy across two contention scenarios and at least eight repetitions each. No model training occurs; Alpamayo is used frozen, and only its runtime memory-scheduling behavior is modified. Estimated total cost: $40 to $150. Model weights and codebase storage are trivial on any rented GPU's disk.
