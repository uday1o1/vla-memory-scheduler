...
Contention scenarios: a steady compute-bound load using a synthetic dummy-workload contention generator (modeled on the general technique in Kutukcu et al., literature-survey.md source 6; their exact terminology was not independently confirmed against the full paywalled text), and a bursty load injected by a second process at randomized intervals, chosen to cover the two qualitatively different contention shapes a real second GPU workload can produce, sustained versus intermittent, rather than only one. Both run as an independent process sharing the GPU through CUDA MPS, chosen over MIG or time-slicing because it allows two independent processes to share one GPU without a hardware repartition, matching the deployment scenario of an unmodified second workload appearing at runtime.
...
**Foundation.** The project forks `aveeslab/oom-free-alpamayo`, whose codebase is small and cleanly modular, with a documented adapter interface and a single function the documentation describes as implementing the GPU-Resident Layer Decision Policy in isolation. The swapping mechanism itself, Sequential and Pipelined Demand Layering, is correct existing infrastructure and is not modified.

**Week-one go or no-go gate, before any policy code is written.** Two facts this project depends on are currently unverified, not assumed true: first, that the documented residency-decision function is actually as cleanly isolated in the real code as its documentation states; second, that CUDA MPS genuinely supports two concurrent processes on whatever GPU is rented, since MPS is not officially supported inside every container platform. Both are confirmed directly on the rented hardware in week one, before committing further budget, and either failing is treated as a real project risk requiring a scope change, not a footnote.
...
Device available: NVIDIA RTX 5050 (desktop, 8GB) for code development only; it cannot hold Alpamayo-R1-10B (22GB) at all, so every actual model-loading, inference, or contention experiment runs on a rented A100-class GPU, not just the final evaluation sweep. Maintainer: Uday Arora; Claude Code access is requested to support implementation throughout the semester.
